...
public class Backup implements IBackup {
    ...
    /**
     * Dies sind alle Ordner die nicht gesichert werden sollen.
     */
    private static List verbOrdner = null;
    ...
    /**
     * Dies ist der Pfad zu der Datei in der der Dateibaum mit letzter änderung
     * staht (für remote)
     */
    private File dateibaumpfad;

    /**
     * Dies gibt an wie viele Versionen behalten werden sollen, wenn overwrite
     * ausgeschaltet ist.
     */
    private int versions;

    /**
     * Dies legt fest, ob ein vollständiges Backup gemacht werden soll, oder nur
     * die veränderten Dateien gesichert werden sollen.
     */
    ...
    private boolean setSichern() {
        try {
            switch (this.zielordner.getProtocol()) {
                case "file":
                    this.sichern = new Lokal(this.quellordner, this.zielordner, this.versions);
                    break;
                case "ftp":
                    this.sichern = new FTP(this.quellordner, this.zielordner, this.versions);
                    break;
                default:
                    log.write("Das Protokoll zum sichern ist unbekannt", FEHLER);
                    return false;
            }
    ...
    /**
     * Dies vergleicht die Dateien die in der Einstellungsdatei waren mit den
     * Dateien in Ordner.
     *
    ...
    static public LinkedList<Path> vergleicheDateien(Properties dateibaum, Path ordnerpath) {
        ...
        for (File datei : dateien) {
            ...
            if (isFolder(path)) {
                ...
                if (getVerboteneOrdner().contains(datei.toString())) {
    ...
    private  static  List getVerboteneOrdner()
    {
        if (verbOrdner != null)
        {
            return verbOrdner;
        }
        String[] verbOrdnerStr = Utils.StringToStringArray(Einstellungen.verbotenerOrdner.getWert().get(), ";");
        verbOrdner = new ArrayList();
        System.out.println(Arrays.toString(verbOrdnerStr));
        for (String pfad : verbOrdnerStr) {
            try {
                verbOrdner.add(Paths.get(new URI(pfad)).toString());
            } catch (URISyntaxException ex) {
                Log.Write("Der angagebene Ordner " + pfad + " kann nicht in eine URL umgewandelt werden.");
            }
        }
        Log.Write("Dies sind alle verbotenen Ordner");
        Log.Write(verbOrdner.toString());
        return verbOrdner;
    }
